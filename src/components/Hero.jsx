...
function Hero() {
  ...
  const stats = [
    { number: "100+", label: "Azure Data Pipelines Deployed", icon: "☁️" },
    { number: "2B+", label: "Records Processed", icon: "📊" },
    { number: "8", label: "Years of Expertise", icon: "⚡" },
  ];
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-8 sm:grid-cols-3 sm:gap-8">
              {stats.map(({ number, label, icon }, index) => (
                ...
                >
                  <span className="block mb-3 text-2xl sm:text-3xl">
                    {icon}
                  </span>
                  <motion.div
                    ...
                    initial={{ number: 0 }}
                    animate={
                      inView ? { number: parseInt(number) } : { number: 0 }
                    }
      ...
      rounded-full overflow-hidden w-full sm:w-auto"
              ...
              {/* Download CV Button */}
              <motion.a
                href="https://drive.google.com/file/d/your-cv-file-id/view?usp=sharing"
                target="_blank"
